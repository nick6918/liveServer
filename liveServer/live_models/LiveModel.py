...
class LiveModel(object):
	...
	def get_live_list(self, dtime):
		exe_string = "SELECT ld.name, ld.title, ld.ctime, ld.dtime, ld.datetime, ml.url, ml.state, ld.vid FROM live_daylist as ld LEFT JOIN m3u8live as ml on ld.vid=ml.vid WHERE ld.datetime = %s"
